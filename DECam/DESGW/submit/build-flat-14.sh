...
#SBATCH -o /shares/soares-santos.physik.uzh/fitsFiles/calibs/logs/cpFlat_14_%j.out
#SBATCH -e /shares/soares-santos.physik.uzh/fitsFiles/calibs/logs/cpFlat_14_%j.err
...
-o DECam/calib/template/flat/14 \
-p $CP_PIPE_DIR/pipelines/DECam/cpFlat.yaml \
-d "instrument='DECam' AND exposure.observation_type='dome flat' AND exposure.day_obs >= 20140000 AND exposure.day_obs <= 20149999" \
2>&1 | tee -a $LOGFILE
date | tee -a $LOGFILE
